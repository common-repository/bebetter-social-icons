		...
		<div class="wrap">
			...
			<td style="vertical-align:top">
				<table class="widefat">
					...
					<tr>
						<td>
							<code>[bb_social_icons icon_size="12" color="#fff" color_hover="#000" transition="300"]</code> These options are available
						</td>
					</tr>
				</table>
			</td>
		...
		if ( is_array( $input['socials'] ) ) {
			foreach ( $input['socials']['icon'] as $k => $icon ) {
				if( $icon != '' && $input['socials']['link'][$k] != '' ) {
					$out[$k]['icon'] = $icon;
					$out[$k]['link'] = esc_url( trim($input['socials']['link'][$k]), array('http', 'https', 'mailto', 'skype', 'tel') );
				}
			}
		}
